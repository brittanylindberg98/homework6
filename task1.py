...
# function that returns number of characters in string
def characterCount(string):
    return len(string)

# function that returns the average word length
def averageWordLen(string):
    # create a list of words
    words = list(string.split())
    sum = 0
    # add the length of all words to variable sum
    for i in words:
        sum += len(i)
    # returning the average len
    return sum / len(words)

# function to return average sentence length
def averageSentenceLen(string):
    # splitting the paragraph to form sentences
    sentences = list(string.split(". "))
    # creating a 2D list of words in sentence 
    answers = []
    for i in sentences:
        answers.append(list(i.split()))
    # storing the sum of all the words in list in variable total
    total = 0
    for i in answers:
        total += len(i)
    # returning the average sentence length
    return total / len(sentences)


def wordDistribution(string):
    # storing the word length in dictionary
    dictionary = {}
    # splitting sentences to for a list of words
    words = list(string.split())
...
# function that stores all the words ending with "ly" in dictionary and returns it
def wordEnding(string):
    dictionary = {}
    words = list(string.split())
    for i in words:
        if i.endswith("ly") or i.endswith("ly.") or i.endswith("ly,") or i.endswith("ly;") :
            dictionary[i] = dictionary.get(i, 0) + 1
    # returning the dictionary
    return dictionary


# function that returns the list of word in order of length non increasing words
def longestWords(string):
    words = list(string.split())
    lengths = []
    for i in words:
        lengths.append(len(i))
    new = zip(words,lengths)
    return sorted(new,key=lambda a:a[1],reverse=True)


# the given string. We can change it as we need
string = """
...
# calling and printing the required functions
print("Number of words are: ",end="")
print(wordCount(string))
print("Number of characters in the string are: ",end="")
print(characterCount(string))
print("The average word length is: ",end="")
print(averageWordLen(string))
print("The average sentence length is: ",end="")
print(averageSentenceLen(string))
print("The word distribution is as follows: ")
dictionary = wordDistribution(string)
for i in dictionary:
    print(i, dictionary[i])
print("Words ending with 'ly' are: ")
dict2 = wordEnding(string)
for i in dict2:
    print(i)
print("Top 10 longest words are: ",end="")
longest=longestWords(string)
for i in longest[:10]:
    print(i[0],end = " ")
